# src/ciphers/aes.py - ПРАВИЛЬНАЯ РЕАЛИЗАЦИЯ AES
"""
AES-128 implementation for GCM.
...
class AES:
    ...
    def __init__(self, key):
        ...
        self.key = key
        self.block_size = 16
        self.nr = 10  # Number of rounds for AES-128
    ...
    def _key_expansion(self, key):
        ...
        nb = 4  # Number of columns in state (fixed at 4 for AES)
        nr = self.nr
        ...
        round_keys = [bytearray(16) for _ in range(nr + 1)]

        # First round key is the original key
        for i in range(nk):
            round_keys[0][4 * i:4 * i + 4] = key_words[i]

        # Generate remaining round keys
        for i in range(nk, nb * (nr + 1)):
            temp = bytearray(round_keys[(i - 1) // 4][4 * ((i - 1) % 4):4 * ((i - 1) % 4) + 4])
            ...
            prev_word = round_keys[(i - nk) // 4][4 * ((i - nk) % 4):4 * ((i - nk) % 4) + 4]
            new_word = bytes([temp[j] ^ prev_word[j] for j in range(4)])

            round_keys[i // 4][4 * (i % 4):4 * (i % 4) + 4] = new_word
