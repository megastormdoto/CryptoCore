...
class TestEncryptThenMAC(unittest.TestCase):
    ...
    def test_key_separation(self):
        ...
        # Они должны быть разными из-за случайного IV в режиме CTR
        # Но дешифрование должно работать
...
class TestGCMIntegration(unittest.TestCase):
    ...
    def test_gcm_cli_basic(self):
        ...
        encrypted_file = tempfile.mktemp(suffix='.enc')
        decrypted_file = tempfile.mktemp(suffix='.dec')

        key = "00112233445566778899aabbccddeeff"
        aad = "aabbccddeeff"

        try:
            # НАЙДЕМ ПРАВИЛЬНЫЙ ПУТЬ К CLI
            ...
            cli_paths = [
                ...
                os.path.join(project_root, 'src', 'cryptocore', 'cli.py'),
            ...
            result = subprocess.run([
                sys.executable, cli_path,  # ИСПРАВЛЕНО
                'encrypt',
                '--key', key,
                '--input', input_file,
            ...
            if result.returncode != 0:
                ...
                print(f"Encryption stdout: {result.stdout}")
            ...
            if result.returncode != 0:
                ...
                print(f"Decryption stdout: {result.stdout}")
