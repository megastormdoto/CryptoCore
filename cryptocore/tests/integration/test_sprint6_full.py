...
class TestSprint6GCM(unittest.TestCase):
    ...
    def _find_cryptocore():
        ...
        possible_names = ['cryptocore.py', 'main.py', 'cli.py']
        possible_dirs = [
            project_root,  # корень проекта
            os.path.join(project_root, 'src'),  # src/
            os.path.join(project_root, 'cryptocore'),  # cryptocore/
            os.path.dirname(project_root),  # родительская директория
        ]
        ...
        print(f"✗ Could not find cryptocore.py")
        print(f"Searched in:")
        for path in paths:
            print(f"  - {path}")
